...
export default function AccountListPage() {
    ...
    useEffect(() => {
        const fetchAccountSummariesAndProperties = async () => {
            try {
                const res = await fetch(`/api/ga4/account-summaries-and-properties?accountId=${accountId}`);
        ...
        fetchAccountSummariesAndProperties();
    }, []);
                        ...
                        <ul>
                            {acc.propertySummaries?.map((prop) => (
                                <li key={prop.property} style={{border: '1px solid #ccc', margin: '1rem 0', padding: '1rem'}}>
                                    <Link href={`/onboarding/accounts/${accountId}/${prop.property}`}>
