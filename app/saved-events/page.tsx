    ...
    useEffect(() => {
        ...
        const fetchSavedEvents = async () => {
            try {
                ...
                // const eventNames = eventDocuments.eventNames;
...
//   };
    ...
    async function fetchEventMetrices() {
        // alert('Fetch Event Metrices clicked');
        // console.log('Fetch Event Metrices clicked');
        // Implement the logic to fetch event metrics here
        // You can use the selectedEvents state to determine which events to fetch metrics for
        try {
            ...
            <ul>
                {savedEvents.map(event => (
                    <li key={event._id} style={{ border: '1px solid #ccc', margin: '1rem 0', padding: '1rem' }}>
                        <p><strong>Account ID:</strong> {event.accountId}</p>
                        <p><strong>Property ID:</strong> {event.propertyId}</p>
                        <p><strong>User ID:</strong> {event.userId}</p>
                        <p><strong>Event Names:</strong> {event.eventNames.join(', ')}</p>
                        <button onClick={fetchEventMetrices} disabled={selectedEvents.size === 0} style={{ marginBottom: '1rem' }}>Fetch</button>
                    </li>
                ))}
            </ul>
